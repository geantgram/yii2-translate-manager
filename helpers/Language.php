...
namespace geantgram\translatemanager\helpers;

use Yii;
use geantgram\translatemanager\services\Scanner;
use geantgram\translatemanager\bundles\TranslationPluginAsset;
...
class Language
{
     ...
     *      ],
     * ];
     * $result = \geantgram\translatemanager\helpers\Language::a($array, $params);
     * ~~~
     *
     * The result:
     *
    ...
    {
        return Yii::$app->session->has(\geantgram\translatemanager\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \geantgram\translatemanager\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \geantgram\translatemanager\models\LanguageSource();
    ...
    {
        $languageSources = \geantgram\translatemanager\models\LanguageSource::find()->select('category')->distinct()->all();
