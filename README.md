...
```
composer require geantgram/yii2-translate-manager
```
...
```
yii migrate/up --migrationPath=@vendor/geantgram/yii2-translate-manager/migrations
```
...
'controllerMap' => [
    'migrate' => [
        ...
        'migrationNamespaces' => [
            'geantgram\translatemanager\migrations\namespaced',
        ],
    ],
],
```
...
'modules' => [
    'translatemanager' => [
        'class' => 'geantgram\translatemanager\Module',
    ],
],
```
...
'modules' => [
    'translatemanager' => [
        ...
        'scanners' => [ // define this if you need to override default scanners (below)
            '\geantgram\translatemanager\services\scanners\ScannerPhpFunction',
            '\geantgram\translatemanager\services\scanners\ScannerPhpArray',
            '\geantgram\translatemanager\services\scanners\ScannerJavaScriptFunction',
            '\geantgram\translatemanager\services\scanners\ScannerDatabase',
        ],
    ],
],
```
...
'components' => [
    'translatemanager' => [
        'class' => 'geantgram\translatemanager\Component'
    ]
]
```
...
```php
\geantgram\translatemanager\helpers\Language::registerAssets();
```
...
use geantgram\translatemanager\helpers\Language;
...
```php
\geantgram\translatemanager\widgets\ToggleTranslate::widget();
```
...
```php
\geantgram\translatemanager\widgets\ToggleTranslate::widget([
 'position' => \geantgram\translatemanager\widgets\ToggleTranslate::POSITION_TOP_RIGHT,
 ...
 'frontendTranslationAsset' => 'geantgram\translatemanager\bundles\FrontendTranslationAsset',
 'frontendTranslationPluginAsset' => 'geantgram\translatemanager\bundles\FrontendTranslationPluginAsset',
]);
```
...
```php
use geantgram\translatemanager\helpers\Language as Lx;
...
**IMPORTANT: The geantgram\translatemanager\helpers\Language::t() (Lx::t()) function currently does not support the translation of HTMLattributes**
...
public function getStatuses() {
    return \geantgram\translatemanager\helpers\Language::a($this->_STATUSES);
}
...
public function getGenders() {
    return \geantgram\translatemanager\helpers\Language::a($this->_GENDERS, 'de-DE');
}
```
...
class Category extends \yii\db\ActiveRecord {
    ...
    public function behaviors()
    {
        return [
            [
                'class' => \geantgram\translatemanager\behaviors\TranslateBehavior::className(),
...
```php
'controllerMap' => [
    'translate' => \geantgram\translatemanager\commands\TranslatemanagerController::className()
],
```
...
* Scanner is scanning parent root directory [#12](https://github.com/geantgram/yii2-translate-manager/pull/12).

  You can overwrite this behavior with the `scanRootParentDirectory` option. (See Config section for details.)
* Frontend translation of strings in hidden tags corrupts HTML. [#45](https://github.com/geantgram/yii2-translate-manager/issues/45)
